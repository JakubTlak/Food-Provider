...
function Ingredients({ ingredients, userName }) {
    ...
    return (
        <div className="ingredient">
            ...
            <ul>
                {ingredients.map(i => <div className="ingredientItem" onClick={() => addIngredient(i.ingredientName)}>{i.ingredientName}</div>)}
            </ul>

            {myIngredients1 ? (myIngredients1.map((i, index) => <div key={index} className="myIngredient">{i}</div>))
                :
